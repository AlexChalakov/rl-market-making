...
def create_cnn_attention_policy_network(input_shape):
    ...
    model.add(layers.Conv1D(128, kernel_size=3, activation='relu', input_shape=input_shape, kernel_initializer='he_uniform'))
    ...
    model.add(layers.BatchNormalization())
    model.add(layers.MaxPooling1D(pool_size=2))
    # Third Conv Layer with BatchNormalization
    model.add(layers.Conv1D(512, kernel_size=3, activation='relu', kernel_initializer='he_uniform'))
    model.add(layers.BatchNormalization())
    # Flattening
...
def create_cnn_attention_value_network(input_shape):
    model = models.Sequential()
    # First Conv Layer with BatchNormalization
    model.add(layers.Conv1D(128, kernel_size=3, activation='relu', input_shape=input_shape, kernel_initializer='he_normal'))
    model.add(layers.BatchNormalization())
    model.add(layers.MaxPooling1D(pool_size=2))
    # Second Conv Layer with BatchNormalization
    ...
    model.add(layers.MaxPooling1D(pool_size=2))
    # Third Conv Layer with BatchNormalization
    model.add(layers.Conv1D(512, kernel_size=3, activation='relu', kernel_initializer='he_normal'))
    model.add(layers.BatchNormalization())
    # Flattening
    model.add(layers.Flatten())
    # Fully Connected Layer with Dropout for Regularization
